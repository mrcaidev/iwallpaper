...
- [3 具体需求](#3-具体需求)
  ...
  - [3.2 功能需求](#32-功能需求)
    - [3.2.1 账号管理](#321-账号管理)
      - [3.2.1.1 登录账号](#3211-登录账号)
      - [3.2.1.2 修改资料](#3212-修改资料)
      - [3.2.1.3 注销账号](#3213-注销账号)
    - [3.2.2 推荐系统](#322-推荐系统)
    ...
    - [3.2.4 壁纸互动](#324-壁纸互动)
      ...
      - [3.2.4.2 收藏壁纸](#3242-收藏壁纸)
      - [3.2.4.3 浏览收藏壁纸](#3243-浏览收藏壁纸)
      - [3.2.4.4 屏蔽壁纸](#3244-屏蔽壁纸)
      - [3.2.4.5 下载壁纸](#3245-下载壁纸)
  - [3.3 性能需求](#33-性能需求)
    - [3.3.1 用户界面](#331-用户界面)
    - [3.3.2 响应时间](#332-响应时间)
    - [3.3.3 可靠性](#333-可靠性)
...
-   用例描述：用户可以通过发送到邮箱的 Magic Link 登录账号。
-   重要级：基本
-   参与者：用户
-   前置条件：用户未登录。
...
-   主事件流：
    ...
    1. 用户访问登录页面。
    2. 用户输入有效的邮箱地址，并且点击登录按钮。
    3. 系统确认邮箱地址有效，将表单置为禁用状态，向用户发送包含 Magic Link 的邮件，并弹窗提示用户查收邮件。
    4. 用户访问发送到邮箱的 Magic Link 指向的页面。
...
-   备选事件流 2 - 用户未收到邮件，或在一分钟内未成功登录：

    1. 用户访问登录页面。
    2. 用户输入有效的邮箱地址，并且点击登录按钮。
    3. 系统确认邮箱地址有效，将表单置为禁用状态，向用户发送包含 Magic Link 的邮件，并弹窗提示用户查收邮件。
    4. 系统一分钟后未检测到用户已登录，将登录表单置为可用状态，并显示帮助“没有收到邮件？”。
...
-   参与者：用户
-   前置条件：用户已登录。
-   后置条件：用户收藏的壁纸被记录，用户可以在收藏页面浏览收藏过的壁纸。
-   主事件流：

    1. 用户浏览壁纸缩略图，或访问壁纸详情页面。
    2. 用户点击收藏按钮。
    3. 系统记录用户收藏了这张壁纸，并调整用户对它的评分。

-   备选事件流 1 - 用户已收藏过该壁纸：

    1. 用户浏览壁纸缩略图，或访问壁纸详情页面。
    2. 用户点击收藏按钮。
    3. 系统记录用户取消收藏了这张壁纸，并调整用户对它的评分。
...
-   主事件流：
    ...
    1. 用户浏览壁纸缩略图，或访问壁纸详情页面。
    2. 用户点击屏蔽按钮。
    3. 系统记录用户屏蔽了这张壁纸，并调整用户对它的评分。

-   备选事件流 1 - 用户已屏蔽过该壁纸：

    1. 用户浏览壁纸缩略图，或访问壁纸详情页面。
    2. 用户点击屏蔽按钮。
    3. 系统记录用户取消屏蔽了这张壁纸，并调整用户对它的评分。
